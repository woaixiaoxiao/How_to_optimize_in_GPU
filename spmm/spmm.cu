...
void ReadFile(std::string& file, int& row_num, int& col_num, int& nnz_num,
              ...
              std::vector<float>& A_value, std::vector<float>& B, std::vector<float>& C)
{
    std::ifstream input;
...
void spmm_cpu_kernel(std::vector<IndexType>& row_offset, std::vector<IndexType>& col_index,
                     std::vector<ValueType>& value, std::vector<ValueType>& B,
                     std::vector<ValueType>& C, IndexType row_num, IndexType col_num)
{
...
__global__ void My_spmm_csr_vector_kernel_v0(const int num_rows, const int* A_row_offset,
                                             const int* A_col_index, const float* A_value,
                                             const float* B, float* C, const int ldb, const int ldc)
{
    // Block index
    int bx = blockIdx.x;
    int by = blockIdx.y;
...
// dim3 dimGrid(row_num/THREAD_NUM_PER_BLOCK, row_num);
// useless optimize
template <const int BLOCK_SIZE_X, const int BLOCK_SIZE_K, const int THREAD_NUM_PER_BLOCK>
__global__ void My_spmm_csr_vector_kernel_v1(const int num_rows, const int* A_row_offset,
                                             const int* A_col_index, const float* A_value,
                                             const float* B, float* C, const int M, const int N,
                                             const int K)
{
    // Block index
    int bx = blockIdx.x;
...
int main(int argc, char** argv)
{
    ...
    */
    ...
    checkCudaErrors(cudaMemcpy(
        d_A_row_offset, A_row_offset.data(), (row_num + 1) * sizeof(int), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(
        d_A_col_index, A_col_index.data(), nnz_num * sizeof(int), cudaMemcpyHostToDevice));
    checkCudaErrors(
        cudaMemcpy(d_A_value, A_value.data(), nnz_num * sizeof(float), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_B, B.data(), B_num * sizeof(float), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(
        d_row_indices, row_indices.data(), row_num * sizeof(int), cudaMemcpyHostToDevice));
    ...
    CHECK_CUSPARSE(
        cusparseCreateDnMat(&matB, col_num, row_num, ldb, d_B, CUDA_R_32F, CUSPARSE_ORDER_ROW))
    // Create dense matrix C
    CHECK_CUSPARSE(cusparseCreateDnMat(
        &matC, row_num, row_num, ldc, d_C_cusparse, CUDA_R_32F, CUSPARSE_ORDER_ROW))
    ...
    // device result check
    CHECK_CUDA(
        cudaMemcpy(C_cusparse.data(), d_C_cusparse, C_num * sizeof(float), cudaMemcpyDeviceToHost))
