...
#define LOAD_FROM_GLOBAL_TO_SHARE(BIG_TILE_INDEX, S_WRITE_STAGE)                             \
    do {                                                                                     \
        ...
        {                                                                                    \
            FETCH_FLOAT4(BS[S_WRITE_STAGE][LOAD_BS_ROW][LOAD_BS_COL]) = FETCH_FLOAT4(        \
...
#define LOAD_FROM_SHARE_TO_REGISTER(R_WEITE_STAGE, S_WRITE_STAGE, SMALL_TILE_INDEX)              \
    do {                                                                                         \
...
#define COMPUTE_SMALL_TILE_BY_TILE()                                                         \
    do {                                                                                     \
        ...
        {                                                                                    \
            if (SMALL_TILE_INDEX != BLOCK_SIZE_K) {                                          \
                LOAD_FROM_SHARE_TO_REGISTER(R_WEITE_STAGE, S_WRITE_STAGE, SMALL_TILE_INDEX); \
            }                                                                                \
...
#pragma unroll
    for (; BIG_TILE_INDEX <= K; BIG_TILE_INDEX += BLOCK_SIZE_K) {
        ...
        if (BIG_TILE_INDEX == 0) {
            LOAD_FROM_GLOBAL_TO_SHARE(BIG_TILE_INDEX, S_WRITE_STAGE);
            LOAD_FROM_SHARE_TO_REGISTER(0, S_WRITE_STAGE, 0);
        }
