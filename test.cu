...
#pragma unroll
    ...
    */
    const int a_tile_index =
        warp_id / 2 * 16 +
        lane_id / 8 *
            4;   // warp_id * 8 + (lane_id / 16)*4; // (warp_id/4)*32 + ((lane_id%16)/2)*4;
    /*
    warp排列如下
    0 1
    2 3
    ...
    */
    const int b_tile_index =
        warp_id % 2 * 32 +
        lane_id % 8 * 4;   //(lane_id % 16) * 4; // (warp_id%4)*16 + (lane_id/16)*8 + (lane_id%2)*4;
// 将A和B的第一个分块从全局内存load到共享内存
...
#pragma unroll
    ...
    __syncthreads();
    // 将当前线程在第一轮计算所需的A的一行和B的一列都读到寄存器中
    ...
    int tile_idx = 0;
    do {
        // 将下一个tile的A和B的小block从全局内存预取到寄存器中，待会存到共享内存中
        // next tile index
...
#pragma unroll
        ...
        // 进入小迭代的计算中，每次先预取小迭代的下一轮的数据，然后计算当前小迭代
#pragma unroll
...
#pragma unroll
    ...
    for (int i = 0; i < 4; i++) {
        FETCH_FLOAT4(
            C[OFFSET(BLOCK_SIZE_M * by + c_block_row + i, BLOCK_SIZE_N * bx + c_block_col, N)]) =
            FETCH_FLOAT4(accum[i][0]);
    }
    // store C01 block
    for (int i = 0; i < 4; i++) {
        FETCH_FLOAT4(C[OFFSET(
            BLOCK_SIZE_M * by + c_block_row + i, BLOCK_SIZE_N * bx + c_block_col + 64, N)]) =
            FETCH_FLOAT4(accum[i][4]);
    }
    // store C10 block
    for (int i = 0; i < 4; i++) {
        FETCH_FLOAT4(C[OFFSET(
            BLOCK_SIZE_M * by + c_block_row + 64 + i, BLOCK_SIZE_N * bx + c_block_col, N)]) =
            FETCH_FLOAT4(accum[i + 4][0]);
    }
    // store C11 block
    for (int i = 0; i < 4; i++) {
        FETCH_FLOAT4(C[OFFSET(
            BLOCK_SIZE_M * by + c_block_row + 64 + i, BLOCK_SIZE_N * bx + c_block_col + 64, N)]) =
